...
백엔드는 Render(Web Service, `Dockerfile` 빌드), 프론트엔드는 Vercel에 배포했다. 별도 DB 서버 없이 뜰 수 있도록 `render` 스프링 프로필에서만 H2 인메모리 DB를 쓰도록 분리했다(로컬 개발은 기존 MySQL 그대로) — 로그인 계정이 재시작/슬립 시 초기화되지만, 게스트 모드로도 전체 기능을 다 쓸 수 있어 캐주얼한 배포 목적엔 충분하다고 판단했다. `server.port`를 `${PORT:8080}`로 바꿔 Render가 동적으로 할당하는 포트에 대응했고, CORS 허용 origin도 하드코딩 대신 `app.cors.allowed-origins` 프로퍼티(환경변수로 오버라이드)로 빼서 로컬/배포 환경을 값만 다르게 설정한다.

### 23. 핸드 도중 입장한 플레이어 격리

**실사용 중 발견된 버그**: 핸드가 진행 중일 때 새로 입장한 플레이어는 그 핸드의 딜을 받은 적이 없는데도 `Player.status` 기본값이 `ACTIVE`라서, "이번 핸드에 아직 살아있는 사람" 집계(폴드승 판정 `applyAction`, 다음 스트리트 액션 순서 `activePlayersFrom`, 쇼다운 대상자 선정 `beginShowdown`/`resolveShowdown`)에 전부 잘못 끼어들었다. 그 결과 원래 인원 중 한 명만 남기고 다 폴드해도 폴드승으로 안 끝나고, 신규 입장자와 쇼다운까지 진행되며 베팅도 가능해지는 문제가 있었다.

`Player`에 `sittingOutThisHand`(이번 핸드는 구경만 하는 중인지) 플래그를 추가하고, `Room.addPlayer`가 핸드 진행 중(`phase`가 있고 `SHOWDOWN`도 아님)에 입장한 사람에게만 표시하도록 했다 — 다음 핸드가 시작되면(`resetForNewHand`) 자동으로 풀린다. 위 5곳의 집계 로직을 `GameEngine.isInCurrentHand(Player)` 헬퍼로 통일해서 같은 실수가 다시 생기지 않게 했다.

### 24. 헤즈업 리빌 결정 타이머 시작 시점 재설계

**실사용 중 발견된 버그**: 프리플랍 올인런아웃처럼 서버가 순간적으로 플랍/턴/리버를 다 처리해버리는 경우, `HeadsUpRevealTimerService`의 결정 제한 시간이 결정권자가 정해진 그 즉시(거의 핸드 시작 직후) 카운트를 시작했다. 반면 프론트는 카드를 순서대로 공개하는 연출을 재생하는 데만 몇 초가 걸려서, 정작 결정 패널이 화면에 뜰 땐 제한 시간이 이미 상당히 줄어있는 문제가 있었다(15번 "나가기" 타이밍 버그와 같은 클래스의 문제 — 아래 "주요 설계 결정" 참고).

`Room`에 `headsUpRevealReadyAtMillis`(클라이언트가 "카드 공개 연출을 다 봤다"고 신호를 보낸 시각)를 추가하고, `GameEngine.markHeadsUpRevealReady(playerId)`가 결정권자 본인의 신호만(중복은 무시) 기록한다. `HeadsUpRevealTimerService`는 결정권자가 정해진 시점이 아니라 이 신호가 기록된 시점부터 실제 카운트다운(5초로 단축)을 시작하도록 재설계했다. 신호가 끝내 안 오는 경우(연결 끊김 등)를 대비해, 결정권자가 정해진 뒤 20초가 지나도 신호가 없으면 신호가 온 것으로 간주하고 강제로 카운트다운을 시작하는 안전장치도 뒀다 — 신호 유실 때문에 게임이 영영 멈추는 걸 막기 위함이다.
...
- **서버 타이머는 "정책이 시작되는 시점"이 아니라 "프론트가 그 결과를 보여줄 수 있는 시점"부터 세야 한다**: 15번(나가기 유예)과 24번(헤즈업 리빌 결정) 둘 다 같은 종류의 버그였다 — 서버 이벤트가 발생한 즉시 시간을 재기 시작했는데, 프론트는 애니메이션 때문에 그 이벤트를 몇 초 뒤에야 화면에 보여줬다. 15번은 프론트 연출 시간(`RESULT_HOLD_MS`)을 백엔드 상수와 맞추는 식으로, 24번은 아예 프론트가 "나 이제 봤어" 신호를 보내야 서버가 타이머를 시작하는 식으로 풀었다 — 후자가 더 정확하지만(연출 길이를 서버가 추정할 필요가 없음) 새 프로토콜 메시지가 필요해서, 상황에 따라 트레이드오프를 다르게 선택했다.
...
| 핸드 오케스트레이션 (odd chip rule, zero-chip 방지, 턴 타이머, 레디/자동시작, 헤즈업 머크, 나가기 유예, GAME OVER 리매치, 방 설정, 강퇴, 좌석 이동, 블라인드 상승/앤티, 핸드 도중 입장자 격리 포함) | `GameEngineTest` | 50 |
