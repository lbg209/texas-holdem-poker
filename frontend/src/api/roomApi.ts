...
// 보내지 않는다 — 서버가 항상 빅블라인드의 절반으로 자동 계산한다.
export function createRoom(
  ...
  return postJson<CreateRoomResponse>('/api/rooms', { name, isPrivate, password, startingChips, bigBlind, maxPlayers });
}
...
export function joinRoom(
  ...
  return postJson<JoinPlayerResponse>(`/api/rooms/${roomCode}/players`, { nickname, authToken, password });
}
...
export function joinRoomByCode(
  ...
  return postJson<JoinPlayerResponse>(`/api/rooms/${roomCode}/players/by-code`, { nickname, authToken });
}
